                        ...
                        <table class="table table-centered table-borderless mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $book->author }}</td>
                                    <td>{{ $book->price }}</td>
                                    <td>{!! $book->published? '<span class="badge bg-success">Publicado</span>' : '<span class="badge bg-danger">Não Publicado</span>' !!}</td>
                                    <td>{{ $book->is_digital? 'Sim' : 'Não' }}</td>
                                    <td class="d-flex flex-column gap-1">
                                        <div>
                                            @if($book->published)
                                            <a href="#" class="rbt-btn-link text-decoration-none" wire:click='publish({{$book->id}},false)'>
                                                <i class="feather-eye me-2"></i>Despublicar
                                            </a>
                                            @else

                                            <a href="#" class="rbt-btn-link text-decoration-none" wire:click='publish({{$book->id}},true )'>
                                                <i class="feather-eye me-2"></i>Publicar
                                            </a>
                                            @endif
                                        </div>
                                        <div>
                                            <a class="rbt-btn-link text-decoration-none" href="{{ route('books.edit', ['slug' => $book->slug]) }}" wire:navigate>
